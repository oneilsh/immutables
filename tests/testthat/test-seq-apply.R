...
testthat::test_that("fapply default keeps only invariant monoids for flexseq", {
...
testthat::test_that("fapply can preserve monoids for flexseq when requested", {
  sum_m <- measure_monoid(`+`, 0, as.numeric)
  x <- as_flexseq(1:4, monoids = list(sum = sum_m))
  y <- fapply(x, function(v) v * 2, preserve_monoids = TRUE)
